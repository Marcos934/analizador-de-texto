<template>
  <nav class="navbar is-info" role="navigation" aria-label="main navigation">
    <div class="navbar-brand">
      <section class="hero is-info">
        <div class="hero-body">
          <p class="title">Análise de Texto</p>
          <p class="subtitle">Com Azure - Cognitive Services</p>
        </div>
      </section>

      <a
        role="button"
        class="navbar-burger"
        aria-label="menu"
        aria-expanded="false"
        data-target="navbarBasicExample"
      >
        <span aria-hidden="true"></span>
        <span aria-hidden="true"></span>
        <span aria-hidden="true"></span>
      </a>
    </div>

    <div id="navbarBasicExample" class="navbar-menu">
      <div class="navbar-end">
        <div class="navbar-item">
          <div class="buttons">
            <a
              class="button is-link"
              href="https://www.linkedin.com/in/marcosmulinarii/"
            >
              <strong>Linked</strong>&nbsp;
              <img
                src="data:image/svg+xml;base64,"
              />
            </a>
            <a class="button is-light" href="https://github.com/Marcos934/analizador-de-texto">
              <strong>Código</strong>&nbsp;
              <img
                src="data:image/svg+xml;base64,"
              />
            </a>
          </div>
        </div>
      </div>
    </div>
  </nav>

  <section class="hero is-small">
    <div class="hero-body">
      <div class="container">
        <slot> </slot>
      </div>
    </div>
    <footer class="footer hero is-medium is-link">
      <div class="content has-text-centered">
        <p>
          Sistema desenvolvido por:
          <strong
            ><a href="https://www.linkedin.com/in/marcosmulinarii/"
              >Marcos V. Mulinari</a
            ></strong
          ><br />
        </p>
      </div>
    </footer>
  </section>
</template>
<script>
import "@/assets/js/bulma-events.js";
export default {};
</script>


<style scoped>
</style>